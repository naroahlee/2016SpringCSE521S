 ...
 */
#define HZ      3299140000      /* second */
#define MHZ     3299140         /* millic second */
#define UHZ     3299            /* micro second */
...
//#define TIMER_START_OFFSET	5000000  /* start OFFSET      */
#define TIMER_START_OFFSET	0  /* start OFFSET      */
//#define INTERVAL_CORR		470000	 /* Timer Error Correction */
#define INTERVAL_CORR		0	 /* Timer Error Correction */
...
print_res(void) {
    ...
    printf("Release         Start           Finish          Delta\n");
	for ( i = 0; i < count; ++i ) {
        printf("%-15lld %-15lld %-15lld %-15lld\n", start_time/MHZ, data[i].dispatch/MHZ, data[i].finish/MHZ, (data[i].finish - data[i].dispatch)/MHZ);
...
main(int argc, char *argv[]) {
    ...
    timerspec.it_interval.tv_nsec = (period % 1000) * 1000000 + INTERVAL_CORR;
    ...
    timerspec.it_value.tv_nsec = now.tv_nsec + TIMER_START_OFFSET;
